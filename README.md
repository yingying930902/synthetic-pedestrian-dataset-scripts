...
- The scripts assume Blender units are metres.
- The camera intrinsics assume a pinhole camera model.
- Lens distortion is not exported.
- The joint scripts export joint locations from a Character Creator armature using a predefined mapping to SMPL-24 joint order.
- The exported joints are not generated by the SMPL model joint regressor.
- The camera-coordinate joint script exports OpenCV-style camera coordinates, not raw Blender camera local coordinates.
- If any required bones are missing, the joint scripts skip those joints and print the missing bone names.
- The scripts do not export 2D image keypoints.

## Suggested dataset description

Frame-wise 3D body-joint coordinates were exported from the animated Character Creator armature in Blender. The exported joints follow a SMPL-24-style joint order using a predefined bone-to-joint mapping. Joint coordinates are provided both in the dataset world coordinate system and in each camera coordinate system following the OpenCV camera convention. Camera intrinsic and extrinsic parameters were exported directly from Blender using the same camera configuration used for rendering.

## License

Please add a license file according to how you intend others to use this repository. For open research code, commonly used options include the MIT License or Apache License 2.0.
